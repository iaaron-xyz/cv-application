...
const MainEducation = ({ data }) => {
  return (
    <div>
      <h2 className="display-font font-bold text-cyan-700 text-xl mb-3">Education</h2>
      {data.education.map((edu, index) => {
        return (
          <div key={index} className="pb-3 mb-2">
            ...
            <div className="leading-none text-teal-600">{edu.optDescription || ''}</div>
          </div>
        )
      })}
    </div>
